...
#relay z_transform, z_offset and z_gradient - done (I think)
...
createDesigninDLFspace<-function(x_low,x_high,in_eqpr,dlInd,dleqInd,test_eqpr,desType,desParam,debug=0)
{
  ...
  if(substr(try(substr(cccc<-try(match.fun(desType),silent=TRUE)[1],1,1),silent=TRUE)[1],1,5)!="Error") flag<-"design Type not found" else
  {
    ...
    MD<-as.matrix(modde$design)
    if(debug) print(head(MD))
    model<-modde$model
    if(debug) print(test_eqpr)
    if(!test_eqpr){					#needs to be augmented
      if(debug) print(head(in_eqpr))
      eqprDes<-matrix(0,ncol=ncol(in_eqpr),nrow=nrow(MD))
      colnames(eqprDes)<-colnames(in_eqpr)
      MD<-cbind(MD,eqprDes)
      SW[colnames(in_eqpr),colnames(in_eqpr)]<-0			#scaling for eqpr set to 0
...
#print(u_high)
  ...
  if(flag=="ok"){
    ...
    if(debug) print(nrow(data.frame(MD_desCandidates)))
...
readDimDoeData<-function(ex_path){
  ...
  tryCatch(dataList$rawRelayCoeffs1<-as.matrix(read.csv(paste(ex_path,"rawRelayCoeffs1.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)

  tryCatch(dataList$rawRelayCoeffs2<-as.matrix(read.csv(paste(ex_path,"rawRelayCoeffs2.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)

  tryCatch(dataList$rawRelayCoeffs3<-as.matrix(read.csv(paste(ex_path,"rawRelayCoeffs3.csv",sep=""), sep=";", dec=",", row.names = 1)),error=function(e) e, warning = function(w) w<-w)
...
#vergl. Funktionen u2uu und uu2u (s.  unten)
  ...
  infosFromU<-list(u_names=u_names,u_roles=u_roles,u_low=u_low,u_high=u_high,u_userunit=u_userunit,u_transform=u_transform,u_offset=u_offset,u_gradient=u_gradient,D=D,V=V)
  return(infosFromU)
}
...
{ 
  ...
  #    denn wahrscheinlich gibt es leere Zeilen, mit denen R nichts anfangen kann
   ...
   #names
  z_names<-as.matrix(Z[,1])
  colnames(z_names)<-"names"
  rownames(z_names)<-Z[,2]
  if(debug) print(head(u_names))
  ...
  infosFromZ<-list(z_names=z_names,z_userunit=z_userunit,z_transform=z_transform,z_offset=z_offset,z_gradient=z_gradient,D_resp=D_resp,V_resp=V_resp)
  return(infosFromZ)
}

provide_u_design<-function(dataList,VR,u_names,u_roles,u_transform,u_offset,u_gradient,u_low,u_high,debug=0,newData=FALSE){
  flag<-"ok"
  factorNames<-u_names
  if(exists("uu_design",where=dataList)&&!newData|exists("uu_new",where=dataList)&&newData) {
    ...
    for(qsI in sort(qusetInd,decreasing=TRUE)) {				#qualitative factors are handled in reverse order
      ...
      if(qRex&inqR){			#transfer levels from qalRel
        ...
        if(debug) print("making columns of 0s and 1s")
        if(debug) print(rownames(u_roles)[qsI])
        #if(debug) print(rownames(factorNames))
    ...
    if(debug) print(paste("deleting char-cols",which(!(colnames(uu_design) %in% rownames(u_roles)))))
    if(any(which(!(colnames(uu_design) %in% rownames(u_roles))))) 
    ...
    if(qRex){
      ...
      if(debug) print(rownames(VR)[remRI])
      VR<-sRfM(VR,-remRI)
      ...
      u<-uu2u(uu_design[,rownames(u_roles)[ctrInd]],sRfM(u_transform,ctrInd),sRfM(u_offset,ctrInd),sRfM(u_gradient,ctrInd))
    } else { 
      if(!newData){	#bei uu_design werden contr, eqpr eingelesen
      ...
      if(debug) print(paste("before call to uu2u",ctrInd))
      uu_design<-uu_design[,rownames(u_roles)[ctrInd]]								####change20181119
	  if(debug) print("still before call to uu2u")
      u<-uu2u(uu_design,sRfM(u_transform,ctrInd),sRfM(u_offset,ctrInd),sRfM(u_gradient,ctrInd),debug=debug)     
    }
    if(u$flag=="ok") u_design<-list(flag=u$flag,u_design=as.matrix(u$u_vec),VR=VR,u_names=u_names,u_roles=u_roles,u_transform=u_transform,
      u_offset=u_offset,u_gradient=u_gradient,u_low=u_low,u_high=u_high ) else u_design<-list(flag=u$flag)
...
provide_u_new<-function(dataList,u_roles,u_transform,u_offset,u_gradient){
...
checkPotential<-function(D,V,u_roles)
{
  ...
  ncnst<-sum(u_roles=="const")+sum(u_roles=="dep")+sum(u_roles=="cdep")+sum(u_roles=="qdep")+sum(u_roles=="ddep") ## change 190111
  ncnstd<-sum((u_roles=="const"|u_roles=="dep"|u_roles=="cdep"|u_roles=="qdep"|u_roles=="ddep")&apply(D,1,function(x) sum(x^2))!=0)  	## change 190111
...
suggestVmatrix<-function(u_roles,D,Vin=as.matrix(0),responses=0,debug=0)
{
  ...
  if(!responses)
    {
      E2<-prepareProjection(u_roles,c("const","cdep","quset","qdep","ddep","scup","contr"),transpose=TRUE) ###changed 180415 again 190102
    } else { 
...
############################################################################################################
#five functions to handle so called "exponent relaying", when material "constants" depend on state variables
############################################################################################################
#creates  relay coefficients from a relay design
fetchRelayCoeffs<-function(relayDesign,u_roles, u_transform, u_offset, u_gradient,middles=TRUE,eps=1e-8,debug=0)
{				#### new function (first half of old makeRelayMatrix) change190106				
  flag<-"ok"			####change180417
  relayCoeffs<-0
  Rsquare<-0

  rDcols<-match(colnames(relayDesign),rownames(u_roles))
  if(debug) print(rDcols)
  if(!any(rDcols)) flag<-"error: no matching columns in relay design" else
  {
    rD_roles<-sRfM(u_roles,rDcols)
    if(debug) print(rD_roles)
    rD_offset<-sRfM(u_offset,rDcols)
    rD_gradient<-sRfM(u_gradient,rDcols)
    rD_transform<-sRfM(u_transform,rDcols)
  
    if(debug) print(colnames(relayDesign))
    if(debug) print(rD_transform)
    relayDesign<-uu2u(relayDesign,rD_transform,rD_offset,rD_gradient)
    if(relayDesign$flag!="ok") flag<-relayDesign$flag else
    {
      relayDesign<-relayDesign$u_vec
      if(debug) print(head(relayDesign))
...
#        relayDesign[,i]<-apply(data.frame(relayDesign[,i],rD_transform[i]),1, function(x) {if(x[2]=="none") as.numeric(x[1]) else log10(as.numeric(x[1]))} )
      ...
      rDcolMiddles<-apply(relayDesign,2,function(x) (max(x)+min(x))/2)
      if(middles) relayDesign<-t(t(relayDesign)-rDcolMiddles) else
        relayDesign<-t(t(relayDesign)-rDcolMeans)
      if(debug) print(head(relayDesign))
      X<-as.matrix(sCfM(relayDesign,rD_roles=="contr"|rD_roles=="scup"|rD_roles=="dep"|rD_roles=="qdep"))   ####change190113
      names(rDcolMeans)[rD_roles=="contr"|rD_roles=="scup"|rD_roles=="dep"|rD_roles=="qdep"]<-paste(colnames(X),"__N",sep="")	####change190102
      names(rDcolMiddles)[rD_roles=="contr"|rD_roles=="scup"|rD_roles=="dep"|rD_roles=="qdep"]<-paste(colnames(X),"__N",sep="")	####change190102
 
      Y<-as.matrix(sCfM(relayDesign,rD_roles!="contr"&rD_roles!="scup"&rD_roles!="dep"&rD_roles!="qdep")) ## i.e. const, cdep or qdep  ####change190111
      names(rDcolMeans)[rD_roles!="contr"&rD_roles!="scup"&rD_roles!="dep"&rD_roles!="qdep"]<-paste(colnames(Y),"__D",sep="")	####change190113
      names(rDcolMiddles)[rD_roles!="contr"&rD_roles!="scup"&rD_roles!="dep"&rD_roles!="qdep"]<-paste(colnames(Y),"__D",sep="")	####change190113

      if(!ncol(X)) flag<-"error: no valid columns in the relay design"  else     ####change180417
      if(det(t(X)%*%X)<eps) flag<-"error: X-matrix is singular" else		####change190106
      {												####change180417
  ...
  #preparing a matrix of  Raw relay Coeffs with rDcolMiddles(10 ^) and the relay coeffs

  rawRelayCoeffs<-matrix(0,nrow=(nrow(relayCoeffs)+1),ncol=(ncol(relayCoeffs)+1))
  
  colnames(rawRelayCoeffs)<-c("normx",colnames(relayCoeffs))
  rownames(rawRelayCoeffs)<-c("normy",rownames(relayCoeffs))
  
  
  if(debug) print(rawRelayCoeffs)
  if(debug) print(colnames(relayCoeffs))
  #taking __D value 
  if(debug) print(names(rDcolMiddles))
  if(debug) print(unlist(strsplit(names(rDcolMiddles),split = "__D")))
  Index_D<-unlist(lapply(strsplit(names(rDcolMiddles),split = "__D"), function(x) which(x ==colnames(relayCoeffs))))+nrow(relayCoeffs)
  if(debug) print(Index_D)
  
#  if(length(which(rD_transform[Index_D]=="log"))==length(Index_D))
#  {
#    rawRelayCoeffs[1,colnames(relayCoeffs)]<-10^(rDcolMiddles[Index_D])
#  }
#  else
#  {
#    rawRelayCoeffs[1,colnames(relayCoeffs)]<-rDcolMiddles[Index_D]
#  }
  Index_Y<-unlist(lapply(colnames(relayCoeffs), function(i) which(i==rownames(rD_offset))))
  if(debug) print(Index_Y)
  if(debug) print(rawRelayCoeffs[1,2:ncol(rawRelayCoeffs)])
  if(debug) print(t(rDcolMiddles[Index_D]))
  if(debug) print(rD_transform)
  if(debug) print(length(rownames(sRfM(rD_transform,Index_Y))))
  if(debug) print(length(colnames(t(rDcolMiddles[Index_D]))))
  
  rawRelayCoeffs[1,2:ncol(rawRelayCoeffs)]<-u2uu(t(rDcolMiddles[Index_D]),sRfM(rD_transform,Index_Y),sRfM(rD_offset,Index_Y),sRfM(rD_gradient,Index_Y),debug=debug)$uu_vec
  if(debug) print(u2uu(t(rDcolMiddles[Index_D]),sRfM(rD_transform,Index_Y),sRfM(rD_offset,Index_Y),sRfM(rD_gradient,Index_Y))$flag)
  if(debug) print(if(debug) print(rawRelayCoeffs[1,2:ncol(rawRelayCoeffs)]))
  
  #transforming to user units
#  for(i in 2:ncol(rawRelayCoeffs))
#  {
#    rawRelayCoeffs[1,colnames(rawRelayCoeffs)[i]]<- rawRelayCoeffs[1,colnames(rawRelayCoeffs)[i]]-rD_offset[which(colnames(rawRelayCoeffs)[i]==rownames(rD_offset))]
#    rawRelayCoeffs[1,colnames(rawRelayCoeffs)[i]]<- rawRelayCoeffs[1,colnames(rawRelayCoeffs)[i]]/rD_gradient[which(colnames(rawRelayCoeffs)[i]==rownames(rD_gradient))]
#  }
  
  
  
  #taking __N values
  if(debug) print(unlist(strsplit(names(rDcolMiddles),split = "__N")))
  Index_N<-unlist(lapply(strsplit(names(rDcolMiddles),split = "__N"), function(x) which(x ==rownames(relayCoeffs))))
  #Index_N<-unlist(lapply(strsplit(names(rDcolMiddles[-unlist(lapply(strsplit(names(rDcolMiddles),split = "__D"), function(x) which( x==colnames(relayCoeffs))))]),split = "__N"), function(y) which(y ==rownames(relayCoeffs))))
  if(debug) print(Index_N) 
#  if(length(which(rD_transform[Index_N]=="log"))==length(Index_N))
#  {
#  rawRelayCoeffs[rownames(relayCoeffs),1]<-10^(rDcolMiddles[Index_N])
#  }
#  else
#  {
#    rawRelayCoeffs[rownames(relayCoeffs),1]<-rDcolMiddles[Index_N]
#  }
  Index_X<-unlist(lapply(rownames(relayCoeffs), function(i) which(i==rownames(rD_offset))))
  if(debug) print(Index_X)
  rawRelayCoeffs[2:nrow(rawRelayCoeffs),1]<-u2uu(t(rDcolMiddles[Index_N]),sRfM(rD_transform,Index_X),sRfM(rD_offset,Index_X),sRfM(rD_gradient,Index_X))$uu_vec 
	  
  #transforming to user units
#  for(i in 2:nrow(rawRelayCoeffs))
#  {
#    rawRelayCoeffs[rownames(rawRelayCoeffs)[i],1]<- rawRelayCoeffs[rownames(rawRelayCoeffs)[i],1]-rD_offset[which(rownames(rawRelayCoeffs)[i]==rownames(rD_offset))]
#    rawRelayCoeffs[rownames(rawRelayCoeffs)[i],1]<- rawRelayCoeffs[rownames(rawRelayCoeffs)[i],1]/rD_gradient[which(rownames(rawRelayCoeffs)[i]==rownames(rD_gradient))]
#  }
#  
  
  #taking relay coeffs
  rawRelayCoeffs[rownames(relayCoeffs),colnames(relayCoeffs)]<-relayCoeffs
  
  
  
  if(!flag=="ok")   relayCoeffs<-list(flag=flag) else   		####change180417
    relayCoeffs<-(list(flag=flag,Rsquare=Rsquare,rawRelayCoeffs=rawRelayCoeffs))
  return(relayCoeffs)		####change180417 & 181220
}

refineRelayCoeffs<-function(rawRelayCoeffs,u_roles, u_transform, u_offset, u_gradient,u_low,u_high,lowScale=1,debug=0)
{				#### new function change190110				
  flag<-"ok"			
  relayCoeffs<-sCfM(sRfM(rawRelayCoeffs,-1),-1)
  rXcols<-match(rownames(relayCoeffs),rownames(u_roles))
  rXcolsScup<-rXcols[sRfM(u_roles,rXcols)[,1]=="scup"]
  rYcols<-match(colnames(relayCoeffs),rownames(u_roles))
  if(debug) print(rXcols)
  if(debug) print(rYcols)

  if(sum(is.na(rXcols))+sum(is.na(rYcols))) flag="error: please change factor or response names in coefficients" else
  {
    if(!any(rXcols)|!any(rYcols)) flag<-"error: no matching columns in raw relay coeffs" else
    {
      rX_roles<-sRfM(u_roles,rXcols)
      if(debug) print(rX_roles)
      rX_offset<-sRfM(u_offset,rXcols)
      rX_gradient<-sRfM(u_gradient,rXcols)
      rX_transform<-sRfM(u_transform,rXcols)
      rXcolMeans<-(sRfM(u_low,rXcols)+sRfM(u_high,rXcols))/2
      if(any(rXcolsScup))
      {
        if(debug) print(rXcolsScup)
        posit<-match("scup",rX_roles)
        if(lowScale) rXcolMeans[posit]<-sRfM(u_low,rXcolsScup) else rXcolMeans[posit]<-sRfM(u_high,rXcolsScup) 
      }
      rY_roles<-sRfM(u_roles,rYcols)
      if(debug) print(rY_roles)
      rY_offset<-sRfM(u_offset,rYcols)
      rY_gradient<-sRfM(u_gradient,rYcols)
      rY_transform<-sRfM(u_transform,rYcols)
    
      if(debug) print( t(sRfM(sCfM(rawRelayCoeffs,1),-1)) )
      if(debug) print(   sCfM(sRfM(rawRelayCoeffs,1),-1)  )

      normX<-uu2u(t(sRfM(sCfM(rawRelayCoeffs,1),-1)),rX_transform,rX_offset,rX_gradient)
      normY<-uu2u(sCfM(sRfM(rawRelayCoeffs,1),-1),rY_transform,rY_offset,rY_gradient)

      if(normX$flag!="ok"|normY$flag!="ok") flag<-paste(normX$flag,normY$flag,sep=" and ") else
      {
	  if(debug) print(normX)
	  if(debug) print(normY)
        normX<-normX$u_vec
	  colnames(normX)<-paste(colnames(normX),"__N",sep="")
        normY<-normY$u_vec
        colnames(normY)<-paste(colnames(normY),"__D",sep="")
	  if(debug) print(normX)
	  if(debug) print(normY)
		  
        rYcolMeans<-normY+(t(rXcolMeans)-normX)%*%relayCoeffs
	  colnames(rYcolMeans)<-colnames(normY)
	  rownames(rXcolMeans)<-colnames(normX)
	  
        if(debug) print(t(rXcolMeans))
        if(debug) print(rYcolMeans)

        #rDcolCtrs<-data.frame(t(rXcolMeans),rYcolMeans)[1,]
		rDcolCtrs<-rYcolMeans
	  
      }
    }
  }
  if(!flag=="ok")   relayCoeffs<-list(flag=flag) else   		####change180417
    relayCoeffs<-(list(flag=flag,rDcolCtrs=rDcolCtrs,relayCoeffs=relayCoeffs))
  return(relayCoeffs)		
}

updateUlimits<-function(u_low,u_high,rDcolCtrs,debug=0)
{
   new_low<-u_low
   new_high<-u_high
   if(debug) print(rownames(u_low))
   if(debug) print(colnames(rDcolCtrs))
   colnames(rDcolCtrs)<-unlist(sapply(colnames(rDcolCtrs),function(x) if(is.na(match(x,rownames(u_roles)))) unlist(strsplit(x,"__"))[1] ))
   if(debug) print(colnames(rDcolCtrs))
   new_low[colnames(rDcolCtrs),1]<- t(rDcolCtrs)
   new_high[colnames(rDcolCtrs),1]<- t(rDcolCtrs)
   return(list(new_low=new_low,new_high=new_high))
}



#creates  relay matrix from relay coefficients
makeRelayMatrix<-function(u_roles,relayCoeffs,debug=0)
{
  flag="ok"											#### new signature change190106
  {											####change180417 - all subsequent rows shifted 2 slots to the right
    ...
    if(debug) print(rownames(relayCoeffs))										  
    if(debug) print(colnames(relayCoeffs))										  
    if(debug) print(rownames(relayMatrix))										  
    ...
    if(debug) print(paste("insertPosV",insertPosV,sep=" is "))															  
    if(debug) print(paste("coeffPosV",coeffPosV,sep=" is "))															
    for(insertPos in sort(insertPosV,decreasing=TRUE)){		#sorting backward makes things easier!
      if(insertPos<nrow(relayMatrix)) { 
        if(rownames(relayMatrix)[insertPos+1]==paste(colnames(relayMatrix)[insertPos],"__N",sep="")){	#name is already there, coeff is just added
          if(debug) print("name is already there, coeff is just added ")																		
          ...
          if(debug) print(paste("row inserted for",paste(colnames(relayMatrix)[insertPos],"__N",sep="")))									#row is inserted, coeff entered
        ...
        if(debug) print(paste("row appended for",paste(colnames(relayMatrix)[insertPos],"__N",sep="")))																									   
        relayMatrix<-rbind(relayMatrix,rep(0,ncol(relayMatrix))) 
        rownames(relayMatrix)[insertPos+1]<-paste(colnames(relayMatrix)[insertPos],"__N",sep="")
        for(coeffPos in coeffPosV){					#no need to sort this time
          relayMatrix[insertPos,coeffPos]<-relayCoeffs[rownames(relayMatrix)[insertPos] ,colnames(relayMatrix)[coeffPos]]
          relayMatrix[insertPos+1,coeffPos]<- -relayMatrix[insertPos,coeffPos]
    ...
    coeffPosV<-unlist(lapply(colnames(relayCoeffs),function(x) {which(x==rownames(relayMatrix))}))
    if(debug) print(coeffPosV)							  
    for(depPos in coeffPosV){		
      if(debug) print(paste("substituing for dependent factor",paste(rownames(relayMatrix)[depPos],"__D",sep="")))																													
      rownames(relayMatrix)[depPos]<-paste(rownames(relayMatrix)[depPos],"__D",sep="")
    }
    relayInfo<-list(flag=flag,relayMatrix=relayMatrix)			
  }									
  return (relayInfo)
}
...
relayVmatrix<-function(u_names,u_roles,u_transform,u_offset,u_gradient,u_low,u_high,V,R=0,Rold=0,rDcolCtrs=0,debug=0){   ##change 190102
  flag="ok"

  if(length(R)==1) if(R==0) {			#if accidentally called initialize R as diagonal of 1's
    R<-diag(nrow(V)) 
    rownames(R)<-rownames(V)
    colnames(R)<-rownames(V)
  }  

  if(length(Rold)==1) if(Rold==0) {		#if no Rold is given initialize Rold as diagonal of 1's	####change 190310
    Rold<-diag(nrow(V)) 										  	####change 190310
    rownames(Rold)<-rownames(V)									  	####change 190310
    colnames(Rold)<-rownames(V)									  	####change 190310
  }  
  
  if (ncol(R)!=nrow(V)|ncol(R)!=nrow(Rold)) flag<-"incompatible R and V" else				####change 190310
  #apply(R,1,function(x) sum(x^2))-1
  {
    V<-R%*%V
    Rtot<-R%*%Rold
  
    sameRows<-intersect(colnames(R),rownames(R))
    if(debug) print(sameRows)
    diffRows<-setdiff(rownames(R),colnames(R))
    if(debug) print(diffRows)
    diffCols<-unlist(lapply(diffRows, function(x) if(substr(x, start=nchar(x)-2, stop=nchar(x))=="__D") gsub("__D","",x) else x))
    diffCols<-unlist(lapply(diffCols, function(x) if(substr(x, start=nchar(x)-2, stop=nchar(x))=="__N") gsub("__N","",x) else x))
    if(debug) print(diffCols)
    
    new_names<-R%*%cbind(rep(0,ncol(R)))
    new_names[sameRows,1]<-u_names[sameRows,]
    new_names[diffRows,1]<-u_names[diffCols,1]
    colnames(new_names)<-colnames(u_names)
    
    new_roles<-R%*%cbind(rep(0,ncol(R)))
    new_roles[sameRows,]<-u_roles[sameRows,]
    new_roles[diffRows,1]<-unlist(lapply(diffRows,function(x) {if(substr(x, start=nchar(x)-2, stop=nchar(x))=="__N") "const" else "dep"}))
    colnames(new_roles)<-colnames(u_roles)
    
    new_transform<-R%*%cbind(rep(0,ncol(R)))
    new_transform[sameRows,]<-u_transform[sameRows,]
    new_transform[diffRows,1]<-u_transform[diffCols,1]
    colnames(new_transform)<-colnames(u_transform)
    
    new_offset<-R%*%cbind(rep(0,ncol(R)))
    new_offset[sameRows,]<-u_offset[sameRows,]
    new_offset[diffRows,1]<-u_offset[diffCols,1]
    colnames(new_offset)<-colnames(u_offset)
    
    new_gradient<-R%*%cbind(rep(0,ncol(R)))
    new_gradient[sameRows,]<-u_gradient[sameRows,]
    new_gradient[diffRows,1]<-u_gradient[diffCols,1]
    colnames(new_gradient)<-colnames(u_gradient)
    
    new_low<-R%*%as.matrix(rep(0,ncol(R)))
    colnames(new_low)<-colnames(u_low)
    new_low[sameRows,]<-u_low[sameRows,]
    new_low[diffRows,1]<- (u_low[diffCols,1] + u_high[diffCols,1])/2 
    if(debug) print("vor der krit Stelle")
    if(debug) print(names(rDcolCtrs))
    if(debug) print(rDcolCtrs)
    if(sum(rDcolCtrs^2)) new_low[names(rDcolCtrs),1]<- t(rDcolCtrs)
    
    new_high<-R%*%as.matrix(rep(0,ncol(R)))
    colnames(new_high)<-colnames(u_high)
    new_high[sameRows,]<-u_high[sameRows,]
    new_high[diffRows,1]<- (u_low[diffCols,1] + u_high[diffCols,1])/2
    if(sum(rDcolCtrs^2)) new_high[names(rDcolCtrs),1]<- t(rDcolCtrs)
  }    
  if(flag!="ok") relay<-list(flag=flag,new_names=u_names,new_roles=u_roles,new_transform=u_transform,new_offset=u_offset,new_gradient=u_gradient,new_low=u_low,new_high=u_high,V=V,R=R,Rtot=R) else	####change 190310
    relay<-list(flag=flag,new_names=new_names,new_roles=new_roles,new_transform=new_transform,new_offset=new_offset,new_gradient=new_gradient,new_low=new_low,new_high=new_high,V=V,R=R,Rtot=Rtot)
  return(relay)
}

relayV_resp<-function(z_names,z_transform,z_offset,z_gradient,V_resp,R_resp=0,debug=0){
  flag="ok"

  if(length(R_resp)==1) if(R_resp==0) {			#if accidentally called initialize R_resp as diagonal of 1's
    R_resp<-diag(nrow(V_resp)) 				####change190111 (the whole block)
    rownames(R)<-rownames(V_resp)
    colnames(R)<-rownames(V_resp)
  } 
  ...
  if (ncol(R_resp)!=nrow(V_resp)) {			####change190111 (the whole if-block)
    flag<-"incompatible R_resp and V_resp"
    relay_resp<-list(flag=flag)
  } else {
    V_resp<-R_resp%*%V_resp  				####change19011: everything from here indented
    ...
    relay_resp<-list(flag=flag,new_names=new_names,new_transform=new_transform,new_offset=new_offset,new_gradient=new_gradient,V_resp=V_resp,R_resp=R_resp)
  }
  return(relay_resp)
}
...
#     then either the most suitable (cosines) or a user alternative (which) are to be used
  ...
  if(!ncol(VR)||qr(sRfM(VDS,u_roles=="contr"|u_roles=="quset"))$rank < ncol(VDS)){    #control factors degenerate   	 	####change180415
    ...
    in_const<-prepareProjection(u_roles,c("const","cdep","qdep","ddep"),origOrd=TRUE)             ####change190113
    in_dep<-prepareProjection(u_roles,"dep")
    in_constDep<-prepareProjection(u_roles,c("const","cdep","qdep","ddep","dep"),origOrd=TRUE)    ####change190113
...
#VD
    ...
    VDbot<-as.matrix(sRfM(VDS,u_roles=="const"|u_roles=="cdep"|u_roles=="qdep"|u_roles=="ddep"|u_roles=="dep"))     ####change190111

    VDB<-cbind(VDS,in_constDep)
    if(sum(u_roles=="const")+sum(u_roles=="cdep")+sum(u_roles=="qdep")+sum(u_roles=="dep")+sum(u_roles=="ddep")) x_roles[(ncol(VDS)+1):ncol(VDB)]<-"const"      ####change190111
#WD
    WDtop<-solve(t(VDtop)%*%VDtop)%*%t(VDtop)        #upper left corner
    WDbot<-(-VDbot)%*%WDtop                          #lower left corner
    WDS<-rbind(WDtop,WDbot)                          #join them
    if(sum(u_roles=="const")+sum(u_roles=="cdep")+sum(u_roles=="qdep")+sum(u_roles=="ddep")+sum(u_roles=="dep")){		   ####change190111
      if(debug) print(rownames(WDS))
      if(debug) print(colnames(VDB))
      WDB<-cbind(WDS,t(VDB)[,u_roles=="const"|u_roles=="cdep"|u_roles=="ddep"|u_roles=="qdep"|u_roles=="dep"])        #add right side      ####change190111
    ...
    if(nrow(VDB)==ncol(VDB)) {    #then RES is empty and scale up is not possible
      ...
      if(debug){
          print(u_roles)
          print(u_roles[u_roles=="contr"|u_roles=="scup"])
          print(cosines)
        }
        if(cosines[u_roles[u_roles=="contr"|u_roles=="scup"]=="scup"] > 1-eps ) {   # ... with cosine 1, it is rejected
          flag<-paste("Scale up is impossibe for", rownames(u_roles)[u_roles=="scup"])
          u_roles[u_roles=="scup"]<-"contr"   
          in_in_scup<-sCfM(in_pot_scup,0)		####change 190102
...
#x_designs: design DL-factors at left, then const-factors, then scup-factors, then eqpr-factors - this can be checked in x_roles
{
  ...
  in_constDep<-prepareProjection(u_roles,c("const","cdep","qdep","ddep","dep"),origOrd=TRUE) ####change 190113
  in_dep<-prepareProjection(u_roles,c("cdep","qdep","ddep","dep"),origOrd=TRUE)		####change 190113
  ...
  if(debug) print(paste("dleqInd",dleqInd))
  if(debug) print(paste("dlInd",dlInd))
  ...
  flag<-modde$flag
  if(debug) print(flag)
  if(flag=="ok"|substr(flag,1,4)=="warn") {					#### change 190103
    ...
    if(variant==1 | variant==2){			  #i.e. u_design
      if(debug) print(colnames(modde$design))
      if(debug) print(colnames(constDepScupDes))
      if(debug) print(rownames(VRES))
      ...
      if(debug){  
        print(colnames(VRES))
        print(colnames(cbind(modde$design,constDepScupDes)))
      }
...
u2uu<-function(u_vec,u_transform,u_offset,u_gradient,debug=0){
  flag<-"ok"
  uu_vec<-u_vec
  colnamesU_vec<-unlist(sapply(colnames(u_vec),function(x) strsplit(x,split = "__D")))
  colnamesU_vec<-unlist(sapply(colnamesU_vec,function(x) strsplit(x,split = "__N")))
  rownamesU_tra<-unlist(sapply(rownames(u_transform),function(x) strsplit(x,split = "__D")))
  rownamesU_tra<-unlist(sapply(rownamesU_tra,function(x) strsplit(x,split = "__N")))
  if(debug) print(paste("inside u2uu",length(rownames(u_transform))))
  if(debug) print(paste("inside u2uu",length(colnames(u_vec))))
  if(debug) print(paste("inside u2uu",rownames(u_transform)))
  if(debug) print(paste("inside u2uu",colnamesU_vec))
  if(length(colnames(u_vec))!=length(rownames(u_transform)))  flag<-"inconsistent RF-number" else
    if(sum(colnamesU_vec==rownamesU_tra)!=ncol(u_vec)) flag<-"inconsistent RF-names" else
...
uu2u<-function(uu_vec,u_transform,u_offset,u_gradient,debug=0){
  flag<-"ok"
  u_vec<-uu_vec
...
prepareAnalysis<-function(u_design,z,z_transform,z_offset,z_gradient,VR,V_resp,debug=0)  
{
  flag="ok"
  nr<-nrow(VR)
  nt<-nrow(V_resp)
  if((nt-nr)!=ncol(z)) {
    ...
    zList<-uu2u(z,sRfM(z_transform,(nr+1):nt),sRfM(z_offset,(nr+1):nt),sRfM(z_gradient,(nr+1):nt),debug=debug)                          	#z in user units gets transformed to SI-units and logs
    z<-zList$u_vec
    flag<-zList$flag
    if(debug) print(flag)
    if(flag!="ok") {
...
analyseDesign<-function(DmodelTemplateList,u_design,z,z_transform,z_offset,z_gradient,VR,V_resp,MD,debug=0)  
#DmodelTempateList is a list of R-formula-models, one for each response
{
  ...
  if((nt-nr)!=ncol(z)) {
    ...
    zList<-uu2u(z,sRfM(z_transform,(nr+1):nt),sRfM(z_offset,(nr+1):nt),sRfM(z_gradient,(nr+1):nt))                          	#z in user units gets transformed to SI-units and logs
    z<-zList$u_vec
    flag<-zList$flag
    if(debug) print(flag)
    if(flag!="ok") {
...
scaleUpProcess<-function(u_new,z_new,fittedModel,u_roles,u_low,u_high,VRES,V_resp,x_low,x_high,validate=TRUE,debug=0)
{
  ...
  in_const<-prepareProjection(u_roles,"const")
  in_dep<-prepareProjection(u_roles,"dep") 
  in_constDep<-prepareProjection(u_roles,c("const","cdep","qdep","ddep","dep"),origOrd=TRUE)   
  in_scup<-prepareProjection(u_roles,"scup")
...
scaleUp_u_levels<-function(u_roles,u_lowIn,u_highIn,VRES,upScale=TRUE)
{
  in_contr<-prepareProjection(u_roles,c("contr","quset"),origOrd=TRUE)  ####change180424
  in_eqpr<-prepareProjection(u_roles,"eqpr")
  in_constDep<-prepareProjection(u_roles,c("const","dep"),origOrd=TRUE)
  in_scup<-prepareProjection(u_roles,"scup")
  numfac<-ncol(in_contr)
  design<-fullFactorial(numfac)
  u_low<-t(cbind(in_contr,in_eqpr))%*%u_lowIn
  u_high<-t(cbind(in_contr,in_eqpr))%*%u_highIn
  u_design<-matrix(ccW(u_low,u_high,0)$cp,nrow=nrow(design),ncol=numfac,byrow=TRUE)+design%*%ccW(u_low,u_high,0)$SW
  constDepScupDes<-constDepScupDesign(u_lowIn,u_highIn,nrow(design),in_constDep,in_scup,useLowForScup=upScale)
  constDepScupDesOut<-constDepScupDesign(u_lowIn,u_highIn,nrow(design),in_constDep,in_scup,useLowForScup=!upScale)
  u_design<-cbind(u_design,constDepScupDes)[,rownames(VRES)]
  x_design<-u_design%*%VRES
  scupInd<-which(colnames(VRES)==rownames(VRES)[which(u_roles=="scup")])
  RES<-sCfM(VRES,scupInd)
  RESred<-t(in_scup)%*%RES     #use slider technique here
  SLI<-(sCfM(constDepScupDesOut,ncol(constDepScupDes))-u_design%*%in_scup)%*%solve(t(RESred))
  u_designOut<-u_design+SLI%*%t(RES)
  u_lowOut<-apply(u_designOut,2,min)
  u_highOut<-apply(u_designOut,2,max)
  return(list(u_lowOut=data.frame(l.low=u_lowOut),u_highOut=data.frame(l.high=u_highOut)))
}

scaleUp_u_design<-function(u_roles,u_design,VRES,upScale=TRUE,scupFactor=1)
{
  in_dummy<-prepareProjection(u_roles,"dummy")  
  in_scup<-prepareProjection(u_roles,"scup")
  in_scup<-prepareProjection(u_roles,"scup",origOrd=TRUE)
#  numfac<-ncol(in_contr)
#  design<-fullFactorial(numfac)
  u_lowIn<-apply(u_design,2,min)
  if(!upScale) u_lowIn[u_roles[,1]=="scup"]<-u_lowIn[u_roles[,1]=="scup"]-log10(scupFactor)
  u_highIn<-apply(u_design,2,max)
  if(upScale) u_highIn[u_roles[,1]=="scup"]<-u_highIn[u_roles[,1]=="scup"]+log10(scupFactor)

#  constDepScupDes<-constDepScupDesign(u_lowIn,u_highIn,nrow(u_design),in_constDep,in_scup,useLowForScup=upScale)
  scupDes<-constDepScupDesign(u_lowIn,u_highIn,nrow(u_design),in_dummy,in_scup,useLowForScup=!upScale)

  x_design<-u_design%*%VRES
  u_designIn<-u_design
  u_designIn[,u_roles=="scup"]<-scupDes
  scupInd<-which(colnames(VRES)==rownames(VRES)[which(u_roles=="scup")])
  RES<-sCfM(VRES,scupInd)
  RESred<-t(in_scup)%*%RES     #use slider technique here
  SLI<-((u_designIn-u_design)%*%in_scup)%*%solve(t(RESred))
  u_designOut<-u_design+SLI%*%t(RES)
#  u_lowOut<-apply(u_designOut,2,min)
#  u_highOut<-apply(u_designOut,2,max)
  return(list(u_designOut=u_designOut))
}
